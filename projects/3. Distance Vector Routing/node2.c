...
void rtinit2() {
    ...
    // get neighbor costs
    neighbor2 = getNeighborCosts(INDEX);

    for (int i = 0; i < neighbor2->NodesInNetwork; i++) {
        dt->costs[i][i] = neighbor2->NodeCosts[i];
    }
    ...
    printdt2(INDEX, neighbor2, dt);

    // create each packet to send
    for (int p = 0; p < MAX_NODES; p++) {
        if (p == INDEX || neighbor2->NodeCosts[p] == INFINITY) continue;  // ensures we aren't sending packet to self.
...
void rtupdate2(RoutePacket* rcvdpkt) {
    ...
    if (TraceLevel == 2) printdt2(INDEX, neighbor2, dt);
}

// print current distance vector
void printCurrentDistanceVector2() {
    int minimums[MAX_NODES];
    calculateMins(&dt2, minimums);
